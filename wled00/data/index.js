...
function populateCustomSegments(s)
{
	console.log(s.seg);
	const pathEls = [
	...
	for (var inst of (s.seg||[])) {
		...
		content += `<input type="checkbox" id="Cseg${i}sel" ${inst.sel ? "checked":""}>`;
		paths += `<path id="wing${i}" style="${inst.sel ? "fill: var(--c-l);":""}" onClick="selWing(${i})" d="${pathEls[i]}"/>`;
	}
	content += 
